...
export class BotTelegramService {
  ...
  private async handleUnbanUser(data: any) {
    ...
    if (!validation.success)
      throw new Error("Invalid data format for unbanuser");
    ...
    await db
      .delete(schema.telegram_ban_users)
      .where(
        and(
          eq(schema.telegram_ban_users.user_telegram_id, user_id),
          eq(schema.telegram_ban_users.ban_from_id, chat_id),
  ...
  async getAllUsersForTelegram() {
    const users = await db.query.users.findMany({
      with: {
        socials: {
          where: eq(
            schema.socials.platform,
            SocialPlatform.enumValues.find((v: string) => v === "telegram") ||
              "telegram",
          ),
        },
        prime: true,
  ...
  async getGroupTopicInfo(groupId: string, name: string) {
    const results = await db
      ...
      .innerJoin(
        schema.telegram_group_infos,
        eq(
          schema.telegram_group_topics.group_id,
          schema.telegram_group_infos.id,
        ),
      )
      .where(
        and(
    ...
    if (!info)
      throw new Error(
        "Group topic info not available for that given name or group id",
      );
    return info;
  }
  ...
  async getUsersByRole(role: "User" | "Admin" = "User") {
    ...
    return users.map((user) => ({
      ...
      prime: user.prime
        ? { status: user.prime.status, expiry: user.prime.expiry }
        : null,
    }));
  }
