...
class UserManager {
  ...
  private constructor() {
    ...
    this.telegramAdaptor = new TelegramAdaptor();
    this.init();
      ...
      if (this.users) {
        Object.keys(this.users).map(async (userId: string) => {
          ...
          if (isprimeUser) {
            ...
            let isuserExistsInChat = await this.telegramAdaptor.getChatMember(
              parseInt(userId),
              chat_id,
            );
            if (isuserExistsInChat && isuserExistsInChat.status === "member") {
              // send them a message to renew their subscription by pushing to queue
              const cbUrl = `${process.env.WEBHOOK_URL}/survertask`;
              const queueManager = QueueManager.getInstance();
              await queueManager.push({
                id: `${userId}_renew_notice`,
                type: "SEND_NOFTIFICATION",
                payload: {
                  cburl: cbUrl,
                  chatId: parseInt(userId),
                  text: "Your Prime subscription has expired. Please renew your subscription to continue using the service.",
                },
              });

              let isbanSuccess = await this.telegramAdaptor.banUser(
                parseInt(userId),
                chat_id,
              );
  ...
  async unbanUsertask(
    ...
    try {
      ...
      let isUserJoined = await this.telegramAdaptor.getChatMember(user_id, chat_id);
      if (isUserJoined && isUserJoined.status === "kicked") {
        let status = await this.telegramAdaptor.UnbanUser(user_id, chat_id);

        if (status && status.result) {
          await this.telegramAdaptor.sendMessage(
